...
<c:set var="route_status_list" value="${sessionScope.route_status}" scope="session"/>
<c:set var="car_state_list" value="${sessionScope.car_state}" scope="session"/>

<c:set var="route_list" value="${sessionScope.route_list}" scope="request"/>
...
<table class="main">
    ...
    <tr>
            ...
            <table>
                <c:if test="${active_route!=null}">
                    ...
                    <tr>
                        <td></td>
                        <td colspan="3">
                            <form name="RouteStatusCarStateChanger" method="post" action="${urlPrefix}/controller"
                                  class="right_side">
                                <input type="hidden" name="command" value="change_route_status_car_state"/>
                                ...
                                <label>${route_status}${colon} <select name="route_status_changer" required>
                                    <c:forEach var="item" items="${route_status_list}">
                                        <c:if test="${item.value!='новый'&&item.value!='запланирован'}">
                                            <option value="${item.value}"
                                                    <c:if test="${active_route.status==item.value}">selected</c:if>
                                            >${item.value}</option>
                                        </c:if>
                                    </c:forEach>
                                </select>
                                ...
                                <br/><br/>
                                <label>${car_state}${colon} <select name="car_state_changer">
                            ...
                            <td colspan="3">
                                <c:if test="${active_route==null}">
                                    <form name="RouteStatusChanger" method="post" action="${urlPrefix}/controller"
                                        ...
                                        <label>${route_status}${colon} <select name="route_status_changer" required>
                                            <c:forEach var="item" items="${route_status_list}">
                                                <c:if test="${item.value!='новый'&&item.value!='выполнен'}">
                                                    <option value="${item.value}"
                                                            <c:if test="${route.status==item.value}">selected</c:if>
                                                    >${item.value}</option>
                                                </c:if>
                                            </c:forEach>
                                        </select>
        ...
        <td class="personal_data">
            ${driver.surname} ${driver.name}<br/>
            ${phone}${colon} ${driver.phone}<br/><br/>
            ${car.model}<br/>
